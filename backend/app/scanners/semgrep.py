...
def run_semgrep(repo_dir: Path) -> List[Finding]:
    ...
    for res in data.get("results", []) or []:
        ...
        severity = raw_severity if raw_severity in ("CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO") else "INFO"
        finding_id = f"semgrep:{check_id}:{path}:{start}"
        
        cwe_data = (extra.get("metadata") or {}).get("cwe", "unknown")
        cwe_category = str(cwe_data[0]) if isinstance(cwe_data, list) and cwe_data else str(cwe_data)
